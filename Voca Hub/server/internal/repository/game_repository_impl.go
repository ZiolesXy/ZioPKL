...
func (r *gameRepository) FindByID(id uint) (*models.Game, error) {
	...
	err := r.db.Preload("Developer").Preload("Difficulty").Preload("Categories").First(&game, id).Error
	if err != nil {
...
func (r *gameRepository) ListAll() ([]models.Game, error) {
	...
	err := r.db.Preload("Developer").Preload("Difficulty").Preload("Categories").Order("created_at desc").Find(&games).Error
	return games, err
}

func (r *gameRepository) ListApproved() ([]models.Game, error) {
	var games []models.Game
	err := r.db.Preload("Developer").Preload("Difficulty").Preload("Categories").Where("status = ?", "approved").Order("created_at desc").Find(&games).Error
	return games, err
}

func (r *gameRepository) ListByDeveloperID(developerID uint) ([]models.Game, error) {
	var games []models.Game
	err := r.db.Preload("Developer").Preload("Difficulty").Preload("Categories").Where("developer_id = ?", developerID).Order("created_at desc").Find(&games).Error
	return games, err
}
